...
$phpmailerReply = new PHPMailer();
    ...
    $phpmailerReply = new PHPMailer\PHPMailer\PHPMailer( true );
}
...
// }
	...
	if(is_array($id)){
		...
		if(!empty($formAllData)){
			...
			$GLOBALS['mailRedirectionLink']=$mailRedirectionLink = $formAllData->redirection_link;
						...
						foreach ($match[0] as $key => $value){ // replacement logic begins
							...
							if(($elementName->element_type != 8)&&($elementName->element_type != 9)&&($elementName->element_type != 10)){
								...
								if($elementName->client_view_multi_select_drop_down == 1){ // hidden form field is passsed only for multiselect dropdown
									...
									$xyz_cfm_alt = implode(",",$xyz_cfm_alt);//1
								}
											...
											if($value == $formAllData->newsletter_email_shortcode) {
												if($elementName->element_type == 2){
													if(is_email($_POST[$elementName->element_name])){
														$newsletterEmail = sanitize_text_field($_POST[$elementName->element_name]);
													}
												}
											}
										}
				...
				foreach ($match[0] as $key => $value){ // server side validations
					...
					if($elementName->element_type == 10){
						...
						if(isset($_POST['g-recaptcha-response'])){
							$secret = get_option('xyz_cfm_recaptcha_private_key');
						...
						if($captchaFlagError == 1){
							$jQueryScript = $jQueryScript.'jQuery("div'.$captchaErrorDivId.'").html("'. __("Captcha verification failed", "contact-form-manager").'");
							return false;
							';
						}
					...
					elseif($elementName->element_type == 8){ //file
						...
						if(((!empty($fileOptionsArray)) && (in_array($extension, $fileOptionsArray))) || ($acceptablefileTypes == "") ){
	...
	// 			print_r($email_to);die;
		...
		try{
				...
				if(!empty($uploadFileNameArray) && $_POST){
...
// 						die;
						...
						$customFieldsArray = json_encode($postArray);

						/*
						 * wp_remote_post - post values to newsletter manager
						* */

						$hash_valid=md5(get_option('xyz_cfm_nlm_code'));
 						if ( is_plugin_active('xyz-wp-newsletter/xyz-wp-newsletter.php') ) {


							$sub_url_pre = get_site_url()."/index.php?wp_nlm=subscribe";
							$response = wp_remote_post( $sub_url_pre, array(
									'method' => 'POST',
									'timeout' => 45,
									'redirection' => 5,
									'httpversion' => '1.0',
									'blocking' => true,
									'headers' => array(),
									'body' => "&xyz_em_email=".rawurlencode($newsletterEmail)."&xyz_newsletter_optinmode=".rawurlencode($newsletterOptinMode)."&xyz_em_mode_choosed=1&xyz_em_listIds=".$newsletterEmailList."&customFieldsDetails=".rawurlencode($customFieldsArray)."&postFrom=cfm&hash=".$hash_valid,
									'cookies' => array()
							)
									);
						...
						if ( is_plugin_active('newsletter-manager/newsletter-manager.php') ) {
							...
							$sub_url_free = get_site_url()."/index.php?wp_nlm=subscribe";
							$response = wp_remote_post( $sub_url_free, array(
									'method' => 'POST',
									'timeout' => 45,
									'redirection' => 5,
									'httpversion' => '1.0',
									'blocking' => true,
									'headers' => array(),
									'body' => "&xyz_em_email=".rawurlencode($newsletterEmail)."&xyz_newsletter_optinmode=".rawurlencode($newsletterOptinMode)."&customFieldsDetails=".rawurlencode($customFieldsArray)."&postFrom=cfm&hash=".$hash_valid,
									'cookies' => array()
							)
									);
					...
					* */
					...
					function xyz_em_allowed_redirect_hosts_confirmation($content){
					    $mailRedirectionLink=$GLOBALS['mailRedirectionLink'];
					    $sourceUrl = parse_url($mailRedirectionLink);
...
echo '<script type="text/javascript">document.location.href="'.$mailRedirectionLink.'"</script>';
					    wp_safe_redirect($mailRedirectionLink);
						//header("Location:".$mailRedirectionLink);
						die;
					}else{
...
echo '<script type="text/javascript">document.location.href="'.$pageURL.'"</script>';
							 wp_redirect($pageURL);
						}else{
							$pageURL .= '?sent=1&form='.str_replace(' ', '', $formAllData->name).$xyz_cfm_form_counter;
              echo '<script type="text/javascript">document.location.href="'.$pageURL.'"</script>';
			...
			if(isset($_GET['sent']) && $_GET['sent'] == 1){
				$xyz_cfm_current_form = sanitize_text_field($_GET['form']);
				$xyz_cfm_current_form = 'xyz_cfm_'.$xyz_cfm_current_form;
				$xyz_cfm_current_form .='_success';

				$formName = sanitize_text_field($_GET['form']);
			...
			if($xyz_cfm_mailSentFlag == 0){ // form display logic
			    ...
			    $classtheme='xyz_cfm_theme_div_id_'.$formAllData->theme_id;



			    $themestyledb = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "xyz_cfm_theme_details WHERE id= %d", $formAllData->theme_id));
			    if (!empty($themestyledb)){
			    $themestyledb = $themestyledb[0];
			    echo '<style type="text/css">'.$themestyledb->content.'</style>';
			    }
			    $messageBody = '<div class="'.$classtheme.'">'.$formAllData->form_content.'</div>';
					...
					foreach ($matches[0] as $key => $value){
						...
						if($formElementDetail->element_type == 10){
							...
							if($formElementDetail->re_captcha == 1){
								$publickey = esc_html(get_option('xyz_cfm_recaptcha_public_key'));
								if($publickey != ''){

								    $xyz_cfm_recaptcha_type=get_option('xyz_cfm_recaptcha_type');
								    if($xyz_cfm_recaptcha_type==1)
								    {
								        $visible='data-size="invisible"';
								        $check="";
								        $call_execute_g="?onload=xyz_cfm_invisible_execute";
								        $async="async defer";
								        $g_execute='<script>var xyz_cfm_invisible_execute = function() {
									...
									$replace = $replace.'<div '.$visible. ' class="g-recaptcha '.$cssClass.'" data-sitekey='.$publickey.'   id="reCaptchaDiv_'.$xyz_cfm_elementId.$xyz_cfm_form_counter.'"  >
								</div>'.$check.' <script type="text/javascript" src="'.$url.$call_execute_g.'" '.$async.'></script>'.$g_execute;
									...
									$replace = $replace.'<span style="color:red;">Configure recaptcha Site & Secret keys</span>';
								}
								$messageBody = str_replace("[".$elementType."-".$elementId."]",$replace,$messageBody);
